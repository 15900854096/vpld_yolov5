    ...
    def __call__(self, p, targets, epoch=0, epochs=1200, i_iter=0):  # predictions, targets
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if (epoch<=epochs*0.7): #RANDOM
                batch_list = torch.arange(0, _bs).repeat(_baseline_samp*_as)
                anchor_list = torch.arange(0, _as).repeat(_baseline_samp*_bs)
                Aselect[batch_list, anchor_list, random.choices(rowlist, k = _baseline_samp*_as*_bs), random.choices(collist, k = _baseline_samp*_as*_bs)] = 1
                Bselect[batch_list, anchor_list, random.choices(rowlist, k = _baseline_samp*_as*_bs), random.choices(collist, k = _baseline_samp*_as*_bs)] = 1
